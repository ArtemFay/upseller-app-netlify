...
// ========== Global state ==========
// User comes from auth-check in index.html (window.__USER__).
...
const state = {
  ...
  zayavki: [],             // all active заявки from /api/podbor/zayavki-list
  clients: [],             // [{name, count}]
  clientFilter: '',
...
async function startZayavka(z) {
  ...
  try {
    const t0 = Date.now();
    const res = await fetch('/api/podbor/load?client=' + encodeURIComponent(z.client));
    if (!res.ok) throw new Error(await res.text());
    const data = await res.json();
...
const SyncQueue = {
  ...
  async flush() {
    ...
    try {
      const res = await fetch('/api/podbor/sync', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ updates: batch }) });
      if (!res.ok) throw new Error(await res.text());
      batch.forEach(u => {
