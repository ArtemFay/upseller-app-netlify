 ...
 * Использует единый сервис-аккаунт-доступ из ../google.js. Один auth-механизм
 * на весь сайт (см. _lib/google.js).
 */

import { getSheets as getSharedSheets } from '../google.js';
...
async function syncToKoroby(reportId, clientName, requestId) {
  ...
  // Arrays (not single entries) so legacy duplicate koroby rows are all addressable.
  // Insertion order = physical row order, so pairing with draftIndex below is deterministic.
  ...
  kAllRows.forEach((raw, i) => {
    ...
    if (!box || !barcode) return;
    const key = box + '|' + barcode;
    if (!korobyIndex.has(key)) korobyIndex.set(key, []);
    korobyIndex.get(key).push({ absRow: KOROBY.startRow + i, data: row });
  });
  ...
  // 4. Index draft: key "box|barcode" → [row, ...] (same array shape as korobyIndex).
  ...
  // Apply one matched draft row to one koroby row (1:1 by physical row order).
  // Extracted as inner closure so the dupe-pairing loop below can reuse it.
  function applyMatchedUpdate(draftRow, korobyEntry) {
    const { absRow, data: oldRow } = korobyEntry;
    const commentParts = [];
  ...
  // Force-zero a koroby row that has no draft counterpart (orphan dupe — shouldn't normally
  // happen because draft is built from koroby, but defensive against drift).
  function applyOrphanZero(korobyEntry) {
    const { absRow, data: oldRow } = korobyEntry;
    const commentParts = [];
    const oldQty = oldRow[K.qtyCol - 1];
    if (Number(oldQty) !== 0) {
      commentParts.push('КОЛ=' + oldQty + '→0');
      batch.push({ range: `'${sn}'!${colLetter(K.qtyCol)}${absRow}`, values: [[0]] });
    }
    const oldStatus = norm(oldRow[K.statusCol - 1]);
    if (oldStatus !== 'СПИСАНО') {
      commentParts.push('СТАТУС=' + oldStatus + '→СПИСАНО');
      batch.push({ range: `'${sn}'!${colLetter(K.statusCol)}${absRow}`, values: [['СПИСАНО']] });
    }
    batch.push({ range: `'${sn}'!${colLetter(K.requestCol)}${absRow}`, values: [[requestId]] });
    const oldComment = norm(oldRow[K.commentCol - 1]);
    let newComment = 'ИНВ ' + requestId + ' (дубль-обнулён)';
    if (commentParts.length > 0) newComment += ': ' + commentParts.join(', ');
    if (oldComment) newComment += ' | ' + oldComment;
    batch.push({ range: `'${sn}'!${colLetter(K.commentCol)}${absRow}`, values: [[newComment]] });
    updatedCount++;
  ...
  for (const [key, draftRows] of draftIndex) {
    const korobyEntries = korobyIndex.get(key) || [];

    if (korobyEntries.length > 0) {
      // MATCHED — pair physical rows 1:1. Both arrays are in sheet-row order.
      const pairCount = Math.min(draftRows.length, korobyEntries.length);
      for (let i = 0; i < pairCount; i++) {
        applyMatchedUpdate(draftRows[i], korobyEntries[i]);
      }
      // Orphan koroby rows (more koroby rows than draft for this key) — zero them.
      for (let i = pairCount; i < korobyEntries.length; i++) {
        applyOrphanZero(korobyEntries[i]);
      }
      // If draft has extras beyond koroby (shouldn't happen — draft is sourced from koroby) — ignore silently.
    } else {
      // NEW BOX — append every draft row for this key
      draftRows.forEach(draftRow => {
        ...
        nr[1] = `${String(today.getDate()).padStart(2, '0')}.${String(today.getMonth() + 1).padStart(2, '0')}.${today.getFullYear()}`;
